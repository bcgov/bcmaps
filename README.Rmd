...
[BC Albers](https://spatialreference.org/ref/epsg/nad83-bc-albers/) projection, which is the B.C. Government standard as `sf` or `Spatial` objects. 
...
> _**IMPORTANT NOTE** Support for Spatial objects (`sp`) is deprecated in {bcmaps} v1.2.0, and will be removed in Summer 2023. Please use `sf` objects with {bcmaps}._
...
> _**IMPORTANT NOTE** All support for Spatial objects (`sp`) is deprecated in {bcmaps} v1.2.0. Please use `sf` objects with {bcmaps}._
...
such as orphaned holes and self-intersections (`class = "sp"` _deprecated_ in v1.2.0)
2. `transform_bc_albers()` for transforming any `sf` or `Spatial` object to 
[BC Albers](https://epsg.io/3005) projection. (`class = "sp"` _deprecated_ in v1.2.0)
3. `self_union()` Union a `SpatialPolygons*` object with itself to remove overlaps, while retaining attributes (_deprecated in v1.2.0_)
