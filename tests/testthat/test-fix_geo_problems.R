...
if (suppressPackageStartupMessages(require(sf))) {
  ...
  test_that("works with self-intersections: sf", {
    ...
    expect_message(out <- fix_geo_problems(st_as_sf(p4)), valid_msg, ignore.case = TRUE, all = FALSE)
    expect_is(out, "sf")
    expect_true(all(st_is_valid(out)))
    expect_message(out <- fix_geo_problems(st_as_sf(p5)), fixing_msg, ignore.case = TRUE, all = FALSE)
    expect_message(out <- fix_geo_problems(st_as_sf(p5)), valid_msg, ignore.case = TRUE, all = FALSE)
    expect_is(out, "sf")
    expect_true(all(st_is_valid(out)))
    expect_message(out <- fix_geo_problems(st_as_sf(p6)), fixing_msg, ignore.case = TRUE, all = FALSE)
    expect_message(out <- fix_geo_problems(st_as_sf(p6)), valid_msg, ignore.case = TRUE, all = FALSE)
    expect_is(out, "sf")
    expect_true(all(st_is_valid(out)))
  })
  ...
  test_that("works with other problems: sf", {
    ...
    expect_message(out <- fix_geo_problems(st_as_sf(p7)), valid_msg, ignore.case = TRUE, all = FALSE)
    expect_is(out, "sf")
    expect_true(all(st_is_valid(out)))
    expect_message(out <- fix_geo_problems(st_as_sf(p8)), fixing_msg, ignore.case = TRUE, all = FALSE)
    expect_message(out <- fix_geo_problems(st_as_sf(p8)), valid_msg, ignore.case = TRUE, all = FALSE)
    expect_is(out, "sf")
    expect_true(all(st_is_valid(out)))
    expect_message(out <- fix_geo_problems(st_as_sf(p9)), fixing_msg, ignore.case = TRUE, all = FALSE)
    expect_message(out <- fix_geo_problems(st_as_sf(p9)), valid_msg, ignore.case = TRUE, all = FALSE)
    expect_is(out, "sf")
    expect_true(all(st_is_valid(out)))
  })
}
