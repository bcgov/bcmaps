...
Next we convert this to a `SpatialPointsDataFrame` using the `sf` package:
...
So let's transform the dataframe of species presence/absences into the same CRS as `bc_bound_data`:
...
```r
plot(st_geometry(bc_bound_data))
plot(spp["spp_present"], expandBB = rep(0.2, 4), graticule = TRUE, add = TRUE)
```
...
#> 10        J          no
#>                      ECOREGION_NAME
#> 1  CENTRAL CANADIAN ROCKY MOUNTAINS
#> 2     BOREAL MOUNTAINS AND PLATEAUS
#> 3                 HAY-SLAVE LOWLAND
#> 4                COLUMBIA HIGHLANDS
#> 5        EASTERN HAZELTON MOUNTAINS
#> 6                    MUSKWA PLATEAU
#> 7                    MUSKWA PLATEAU
#> 8         THOMPSON-OKANAGAN PLATEAU
#> 9                      FRASER BASIN
#> 10 CENTRAL CANADIAN ROCKY MOUNTAINS
#>                    geometry
